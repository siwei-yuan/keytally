// Percent Skog Reboot usage-lights keymap(VIA + usage lights)
// 基于 scottywei fork 的 default keymap(update-and-add-bioi-keyboards 分支),
// 仅追加 usage_lights 集成;原键位、BLE、RGB 控制逻辑不动。
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    if (!process_record_usage_lights(keycode, record)) return false;
    switch (keycode) {
        case KC_F21:
